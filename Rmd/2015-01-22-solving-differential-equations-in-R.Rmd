...
Marc Mangel, famed quantitative ecologist, is offering a course in Quantitative Fisheries at NMFS' Southwest Fisheries Science Center
...
familiar with the sorts of dynamical systems that we will be developing in Marc's class.  Not only that, but there are huge, gaping
holes in my formal mathematics background: I never took a course in ODE's or PDE's as an undergraduate, for example.  And finally,
...
develop some intuition about these problems (and maybe even reach some of the dusty corners of my mind.)
...
### quickly doing logistic growth
```{r}
library(deSolve)
parameters <- c(r = 0.3, K = 10000)
state <- c(N = 150)
...
recovery occurs (i.e. harvest is 0) or harvest is $0.1 C_{max}(N(t))$, $0.2 C_{max}(N(t))$, $\ldots$, $0.9 C_{max}(N(t))$.
...
All of this could be one pretty easily, analytically, but I
won't to play around with `deSolve` so let's just be silly about
...
just going to be $rN(1-N/K)$.  Therefore we can easily compute a `LogistFish` function that
has a parameter `fract_of_max`, which we will call `fom` for short:
```{r}
